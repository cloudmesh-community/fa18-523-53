...
| code: [:cloud:](https://github.com/cloudmesh-community/fa18-523-53/tree/master/project-code)
...
Optionally each of these messages can be given with a key and whose hash values determines the partition it should be saved in. Hence the messages with the same key value will be stacked together in the same partition. +@fig:topicWithMultiplePartitions [@www-kafkaGuide] describes four partitions of a single topic.
...
+@fig:kafkaArchitecture describes the high level Kafka Architecture

![Kafka Architecture](images/kafkaArchitecture.png){#fig:kafkaArchitecture}
...
+@fig:scoringApplicationDesign describes how the above stated microservices are orchestrated in Kafka.

![Scoring Application Design](images/scoringApplicationDesign.png) {#fig:scoringApplicationDesign}
...
This command opens a user interface to key in the personal identification information. Provide the details and hit "Check your Score" button. +@fig:scoringInputUI is the sample input screen.
