...
Optionally each of these messages can be given with a key and whose hash values determines the partition it should be saved in. Hence the messages with the same key value will be stacked together in the same partition. +@fig:topicWithMultiplePartitions describes four partitions of a single topic.

![Representation of topic with multiple partitions [@www-kafkaGuide].](images/kafkaPartitions.png){#fig:topicWithMultiplePartitions}
